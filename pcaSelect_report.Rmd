...
Inputting required data of interest:
...
Here we have initial PCA plots:
...
Here we have the PCA loadings:

```{r, echo=FALSE, warning=FALSE}
if (method=='princomp') {
  ###################################################################
  ## compare loadings
  knitr::kable(pca$loadings[,])
#   pca$loadings
  ###################################################################
}
```
...
Next, we plot the variances of each component. We want to select the number of PCs that are the elbow:
...
Next, is a plot of the individuals and their PC1 and PC2 scores plotted:
...
Continuing, there are three plots showing two examples of the pca scores as a result of different variables. The last plot shows how the PCA scores based on PC1.
...
Plots of the most important variables vs. PCA PC1 scores:
...
Plots of the most important variables vs. PCA PC2 scores:
...
Most important variables based on inclusion of supplementary information:
...
Supplementary data of variable contributions:
...
if (method=='FactoMineR') {
  if (exists("suppData") || example=='YES') {
    ...
    names(sort(dimsum, decreasing=T))
    ###################################################################
  }
}
```
...
##If no supplementary information...Biplot:
...
Variable plots if no supplementary information:
